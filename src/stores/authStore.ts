...
export const useAuthStore = defineStore('auth', {
    ...
    actions: {
        async signup(user: any) {
            ...
            this.error = null;
            try {
                const response = await axios.post('https://127.0.0.1:8000/api/signup', user);
                ...
                toast.add({ severity: 'error', summary: 'Erreur', detail: this.error, life: 3000 });
                throw error;
            } finally {
                this.loading = false;
            }
        ...
        async forgotPassword(email: string) {
            try {
                const response = await axios.post(`https://127.0.0.1:8000/api/forgot-password`, { email });
                t
        ...
        async resetPassword(token: string, userData: { password: string, passwordConfirmation: string }){
            try {
                const response = await axios.patch(`https://127.0.0.1:8000/api/reset-password/${token}`, userData );
               return response.data;
            } catch (err) {
        ...
        async verifyEmail (token: string) {
            try {
                await axios.get(`https://127.0.0.1:8000/api/verify/${token}`);
            } catch (err) {
